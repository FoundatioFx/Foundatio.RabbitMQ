# Agent Guidelines for Foundatio.RabbitMQ
...
```bash
# Build
dotnet build Foundatio.RabbitMQ.slnx

# Test
dotnet test Foundatio.RabbitMQ.slnx

# Format code
dotnet format Foundatio.RabbitMQ.slnx
```
...
```text
src
└── Foundatio.RabbitMQ                # RabbitMQ implementations
    ├── Messaging                     # IMessageBus implementations for RabbitMQ
    └── Extensions                    # Extension methods for DI
tests
└── Foundatio.RabbitMQ.Tests          # Unit and integration tests
...
1. **Builds successfully**: `dotnet build Foundatio.RabbitMQ.slnx` exits with code 0
2. **All tests pass**: `dotnet test Foundatio.RabbitMQ.slnx` shows no failures
...
# Specific test file
dotnet test --filter "FullyQualifiedName~RabbitMqMessageBusTests"
